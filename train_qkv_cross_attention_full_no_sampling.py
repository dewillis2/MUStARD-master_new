...
MAX_CONTEXT_TEXT_LEN = 256

# 完整 audio / video 序列：
# 不截断，不均等采样
USE_FULL_AUDIO_VIDEO = True

# 如果本地爆内存，先改成 100 测试
# 全量训练设为 None
MAX_SAMPLES = None

# 为了尽量防止完整序列爆显存，这里先用 64
# 如果能跑，可以改成 128
HIDDEN_DIM = 64
NUM_HEADS = 4
...
def safe_torch_load(path):
    """
    使用 mmap=True 尽量减少 CPU 内存压力。
    如果当前 torch 版本不支持 mmap=True，则自动退回普通 torch.load。
    """
    print(f"读取文件: {path}")
    try:
...
def masked_mean(x, mask):
    ...
    只对真实 token / time step 做平均，不平均 padding。
    """
...
# Dataset
# =========================================================
class FullNoSamplingDataset(Dataset):
    """
    完整序列版 Dataset。

    特点：
    1. text 使用 token-level feature
    2. audio 使用完整 wav2vec2 sequence
    3. video 使用完整 ResNet frame sequence
    4. 不做前方截断
    5. 不做均等采样
    """
...
# Model
# =========================================================
class FullNoSamplingQKVModel(nn.Module):
    """
    完整序列 Q-K-V cross-modal attention。
    ...
    Q = text token
    K,V = 完整 audio sequence / 完整 video frame sequence
    """
