...
MAX_CONTEXT_TEXT_LEN = 192

MAX_UTTER_AUDIO_LEN = 300
MAX_CONTEXT_AUDIO_LEN = 500

MAX_UTTER_VIDEO_LEN = 12
MAX_CONTEXT_VIDEO_LEN = 24
...
def uniform_sample_seq(x, max_len):
    """
    从整个序列中均匀采样 max_len 个时间步，而不是只取前 max_len 个。

    x: [seq_len, dim]

    例如：
    原来 context_audio 是 [800, 768]
    max_len = 500
    那么会从 0 到 799 均匀抽 500 个位置。
    这样可以保留前中后的信息。
    """
    seq_len = x.shape[0]
...
# Dataset
# =========================================================
class FullQKVUniformDataset(Dataset):
    def __init__(self):
        print("正在读取特征文件...")
    ...
    def __getitem__(self, idx):
        ...
        # audio：均匀采样，不是只取前面
        utter_audio = uniform_sample_seq(
            self.ua[sid]["audio_feature"],
            MAX_UTTER_AUDIO_LEN
        ).float()

        context_audio = uniform_sample_seq(
            self.ca[sid]["context_audio_feature"],
            MAX_CONTEXT_AUDIO_LEN
        ).float()

        # video：均匀采样
        utter_video = uniform_sample_seq(
            self.uv[sid]["video_feature"],
            MAX_UTTER_VIDEO_LEN
        ).float()

        context_video = uniform_sample_seq(
...
# Model
# =========================================================
class FullQKVUniformModel(nn.Module):
    """
    full context Q-K-V cross-modal attention。
